...
//            jedis.auth("123456");
     ...
     * @return
     */
    public String setSeckillNumber(long seckillId, int number) {
        try {
            Jedis jedis = jedisPool.getResource();
//            jedis.auth("123456");
            try {
                //key
                String key = "seckillNumber:" + seckillId;
                int timeout = 60 * 60 * 24; //一小时
                String result = jedis.setex(key, timeout, "" + number);
    ...
    /**
     * 根据id获得相应秒杀商品的库存数量，并返回
     * @param seckillId
     * @return
     */
    public String getSeckillNumber(long seckillId) {
        try {
            Jedis jedis = jedisPool.getResource();
//            jedis.auth("123456");
            try {
     ...
     * @return
     */
    public Long decreaseNumber(long seckillId) {
        try {
            Jedis jedis = jedisPool.getResource();
//            jedis.auth("123456");
            try {
